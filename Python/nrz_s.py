...
# ========== NRZ-S Encoding Function ==========
def nrzs_encode(bits):
    """NRZ-S encoding: 0 -> invert level, 1 -> keep previous"""
    encoded = []
    current = 0  # starting level (0)
    for b in bits:
        if b == 0:
...
def main():
    ...
    # Apply NRZ-S encoding
    encoded = nrzs_encode(bits)
    ...
    plt.title("Input Bitstream (0/1)")

    # ---- NRZ-S Encoded Signal ----
    plt.subplot(2,1,2)
    ...
    plt.xlim(0, n)
    plt.grid(True, linestyle='--', linewidth=0.7, color='gray')
    plt.title("NRZ-S Encoded Signal (0/V)")
